...
def scheduling(meta_config):
    ...
    logger.info("{}--> This machine is the {} of group {}".format(current_script, role, current_machine_group))
    ...
    while True:
        ...
        if (current_datetime.hour == meta_config["general_config"]["resume_time_hour"]) or meta_config["general_config"]["resume_time_hour"] == -1:
            print("\n------------------------------------")
            logger.info("{}--> Time to entering scheduling, data collection start".format(current_script))
            if role ==  "client":
            ...
            if role == "server":
                ...
                scheduling_port_zero = scheduling_port_zero + 1
            logger.info("{}--> All test done Successfully~~".format(current_script))
            print("------------------------------------\n")
        else:
            logger.info("{}--> Scheduling will start at {} o'clock, Now is {} o'clock~~".format(current_script, meta_config["general_config"]["resume_time_hour"], current_datetime.hour))
...
def scheduling_client(meta_config, schedule_profile_list, current_machine_group, communication_port):
    ...
    while index < len(schedule_profile_list):
        ...
        for key in main_config:
            task_name = key
        logger.info("{}--> Run Experiment: {}, {}, {}".format(current_script, main_config[task_name]["network"], task_name, main_config[task_name]["variant"]))
        with open(temp_config_file, 'w') as f:
            json.dump(main_config, f, indent = 2)
        client_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        if not my_socket.retry_connect(client_socket, server_address_port):
            logger.error("{}--> Connect Fail, redo secheduling".format(current_script))
            time.sleep(60)
            continue
        if not my_socket.retry_send(client_socket, (json.dumps(main_config) + "##DOKI##").encode("utf-8")):
            logger.error("{}--> Send Fail, redo secheduling".format(current_script))
            time.sleep(60)
            continue
...
def scheduling_server(meta_config, schedule_profile_list, current_machine_group, communication_port):
    ...
    while True:
        logger.info("{}--> Wait client message to start the experiment".format(current_script))
        client_socket, client_address = server_socket.accept()
        logger.debug("{}--> Recieve from client {}".format(current_script, client_address))
        message = my_socket.doki_wait_receive_message(client_socket)
        if message == None:
            logger.error("{}--> Recieve client message Error! Redo scheduling".format(current_script))
            continue
        if message == "scheduling_end":
            ...
            if not my_socket.retry_send(client_socket, ("scheduling_start" + "##DOKI##").encode("utf-8")):
                continue
            logger.debug("{}--> SYN with client successfully, save client config and start to run experiment..".format(current_script))
