...
def upload_iperf_wireshark(main_config=None):
    ...
    for i in range(0, total_run):
        ...
        if selected_variant == "udp":
            os.system("iperf3 -c {} -p {} --length 1472 -u -b {}m -t {} -i {} 2>> {}  &".format(server_ip, server_iperf_port, udp_sending_rate, task_time, iperf_logging_interval, output_iperf_log))
            time.sleep(task_time + time_flow_interval)
            os.system('killall iperf3 2> /dev/null')
        if selected_variant != "udp":
            os.system("sudo sysctl net.ipv4.tcp_congestion_control={}".format(selected_variant))
            os.system("iperf3 -c {} -p {} -t {} -i {} 2>> {} &".format(server_ip, server_iperf_port, task_time, iperf_logging_interval, output_iperf_log))
            time.sleep(task_time + time_flow_interval)
            os.system('killall iperf3 2> /dev/null')
...
def download_iperf_wireshark(main_config=None):
    ...
    for i in range(0, total_run):
        ...
        output_iperf_log = os.path.join(iperf_logging_path, "{}_{}_{}_{}.log".format(selected_network, selected_direction ,selected_variant, current_datetime.strftime("%Y_%m_%d_%H_%M")))
        if selected_variant == "udp":
            os.system("tcpdump -i any udp port {} -w {} 2>> /dev/null &".format(server_iperf_port, output_pcap))
            os.system("iperf3 -c {} -p {} -R --length 1472 -u -b {}m -t {} -i {} 2>> {} &".format(server_ip, server_iperf_port, udp_sending_rate, task_time, iperf_logging_interval, output_iperf_log))
            time.sleep(task_time + time_flow_interval)
            os.system('killall iperf3 2> /dev/null')
            os.system('killall tcpdump 2> /dev/null')
            os.system("python3 my_subprocess.py pcap2txt --mode udp --file-path {} &".format(output_pcap))
        if selected_variant != "udp":
            os.system("tcpdump -i any tcp src port {} -w {} 2>> /dev/null &".format(server_iperf_port, output_pcap))
            os.system("iperf3 -c {} -p {} -R -t {} -i {} 2>> {} &".format(server_ip, server_iperf_port, task_time, iperf_logging_interval, output_iperf_log))
            time.sleep(task_time + time_flow_interval)
            os.system('killall iperf3 2> /dev/null')
            os.system('killall tcpdump 2> /dev/null')
